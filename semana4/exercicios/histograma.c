...
void createHistogram(int c, int h[c]) {
    ...
    double intervalo = 10 / c;

    // valor do intervalo
    int quantidade;
    scanf("%i", &quantidade);

    // o primeiro for serve para ler os valores númericos que o usuário digitar
    for (int i = 0; i < quantidade; i++) {
        double valor;
        scanf("%lf", &valor);
        int index = valor / intervalo;

        //  serve para colocar o valor numérico na sua posição correspondente na lista
        if (index == c) {
            h[c - 1] += 1;
        } else {
            h[index] += 1;
